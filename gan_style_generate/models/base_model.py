...
class BaseModel(ABC):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if dist.is_initialized():
                    # 多卡训练时用 DistributedDataParallel 包装网络。
    ...
    def get_current_visuals(self):
        """Return visualization images. train.py can save these images to disk."""
    ...
    def get_current_losses(self):
        ...
        for name in self.loss_names:
            if isinstance(name, str):
                ...
                errors_ret[name] = float(loss_value)  # float(...) works for both scalar tensor and float number
        return errors_ret
    ...
    def save_networks(self, epoch):
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            for name in self.model_names:
                if isinstance(name, str):
                    # 保存格式：latest_net_G_B.pth、5_net_D_A.pth 等。
