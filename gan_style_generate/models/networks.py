...
def get_norm_layer(norm_type="instance"):
    ...
    """
    # CycleGAN 默认使用 InstanceNorm，它通常比 BatchNorm 更适合风格迁移。
    if norm_type == "batch":
...
def get_scheduler(optimizer, opt):
    ...
    """
    # 默认 linear：先保持学习率，再线性衰减，和 CycleGAN 常见训练设置一致。
...
def init_weights(net, init_type="normal", init_gain=0.02):
    ...
    We use 'normal' in the original CycleGAN setting. But xavier and kaiming might
...
def define_G(input_nc, output_nc, ngf, netG, norm="batch", use_dropout=False, init_type="normal", init_gain=0.02):
    ...
    # 默认 resnet_9blocks：先下采样、经过 9 个 ResNetBlock、再上采样回图像。
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm="batch", init_type="normal", init_gain=0.02):
    ...
    # 默认 basic 是 70x70 PatchGAN，判断局部 patch 的真假，适合学习纹理风格。
...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real):
        ...
        Returns:
            the calculated loss.
        """
        # lsgan 使用 MSELoss；vanilla 使用 BCEWithLogitsLoss；wgangp 用均值形式。
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type="reflect"):
        ...
        # 生成器结构：输入卷积 -> 下采样 -> ResNet 残差块 -> 上采样 -> Tanh 输出。
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        # PatchGAN 输出的是一张真假评分图，而不是单个真假标量。
        kw = 4
        padw = 1
